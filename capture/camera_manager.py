...
class FeedState:
    config: FeedConfig
    status: str = "stopped"  # stopped|connecting|live|degraded|disconnected|ended
...
class CameraManager:
    ...
    def get_frame(self, feed_id: str) -> Optional[np.ndarray]:
        ...
        # Implement frame synchronization to reduce lag between input and output
        frame_sync = st.session_state.get("frame_sync", {})
        if frame_sync.get("enabled", False):
            # If we have too many frames queued, drop frames to catch up
            if frame_sync.get("drop_frames_on_lag", True):
                # Check if we need to skip frames to catch up
                now = time.time()
                last_sync = frame_sync.get("last_sync_time", 0)
                
                # If we're processing frames too slowly, skip some to catch up
                if now - last_sync > 0.5:  # If we're more than 0.5s behind
                    # Update last sync time to now
                    frame_sync["last_sync_time"] = now
                    
                    # Skip intermediate frames by getting the latest frame
                    for _ in range(3):  # Try to get a more recent frame
                        if hasattr(state, 'capture') and state.capture:
                            try:
                                ret, new_frame = state.capture.read()
    ...
    def _open_capture(self, state: FeedState):
        ...
        if feed_type == 'webcam':
            ...
            # Set buffer size to 1 to avoid latency buildup
        ...
        # RTSP handling ---------------------------------------------------
        if feed_type == 'rtsp':
            # Set FFMPEG options to improve RTSP streaming
            ...
            while not state.stop_flag:
                try:
                    ...
                    while not state.stop_flag:
                        ...
                        try:
                            # Skip frames if we're falling behind (read twice; keep latest)
                            # This helps when source is faster than our FPS cap
                            elapsed_since_last = time.time() - state.last_frame_time if state.last_frame_time > 0 else 0
                            if elapsed_since_last > frame_interval * 2 and state.frame_count > 0:
                                # We're falling behind, skip a frame to catch up
                                _ = cap.read()  # Skip a frame
                            
                            ok, frame = cap.read()

                            if not ok or frame is None or frame.size == 0:
                                # For FILE sources: loop instead of stopping after EOF (if we read at least one frame)
                                ...
                                if consecutive_failures >= max_consecutive_failures:
                                    print(f"Too many consecutive failures for {state.config.id}, marking as disconnected")
                            ...
                            # FPS cap - only sleep if we're ahead of schedule
                    ...
                    # If we got here, the inner loop broke - either due to stop_flag or errors
                    if state.stop_flag:
                        # User requested stop, exit the outer loop too
                        break
                    
                    # Otherwise, we'll try to reconnect after cleaning up
                    print(f"Feed {state.config.id} disconnected, attempting to reconnect...")
                    ...
                    # Backoff before reconnection attempt
                    time.sleep(2.0)
                    
                except Exception as open_error:
                    print(f"Failed to open feed {state.config.id}: {open_error}")
                    state.status = 'disconnected'
                    state.error = str(open_error)
                    # Backoff before retry
                    time.sleep(2.0)  # Wait 2 seconds before attempting to reconnect
